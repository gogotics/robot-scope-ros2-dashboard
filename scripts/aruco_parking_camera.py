...
def main():
    ...
    while time.monotonic()-start<a.seconds:
        ...
        corners,ids,_=detector.detectMarkers(cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)); detected=set()
        if ids is not None:
            for cs,mid in zip(corners,ids.reshape(-1)):
                ...
                try:
                    ...
                    label_color=(255,0,0) if error<=a.max_reprojection_error else (0,165,255)
                    cv2.putText(frame,f"err {error:.1f}px",(int(x),min(h-8,int(y)+22)),cv2.FONT_HERSHEY_SIMPLEX,.45,label_color,1)
